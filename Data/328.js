...
// start - all //
    const fileOps = async () => {
        try {
            const data0 = await fspromises.readFile("328 Config/all.txt", "utf8");
            await fspromises.writeFile(enddest, start);
            await fspromises.appendFile(enddest, data0);
// end start - all //

// backup //
            const bup1 = await fspromises.readFile("328 Config/backup1.txt", "utf8");
            await fspromises.appendFile(enddest, "\n\n" + bup1 + bup2);
            const bup3 = await fspromises.readFile("328 Config/backup3.txt", "utf8");
            await fspromises.appendFile(enddest, "\n" + bup3);
// end backup //

// vlans //
            const data1 = await fspromises.readFile("328 Config/vlansall.txt", "utf8");
            await fspromises.appendFile(enddest, "\n\n" + data1 + "\n");
            for (let x = 1000; x <= vlans; x++) {
                await fspromises.appendFile(enddest, "add bridge=bridge tagged=bridge,ether1,ether2,ether3,ether4,ether5,ether6,ether7,ether8,ether9,ether10,ether11,ether12,ether13,ether14,ether15,ether16,ether17,ether18,ether19,ether20,ether21,ether22,ether23,ether24,sfp-sfpplus1,sfp-sfpplus2,sfp-sfpplus3,sfp-sfpplus4 vlan-ids=" + x + "\n");
            }
